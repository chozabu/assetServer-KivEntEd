...
def factory(methodname, cls=dict, flag=True):
	def method(self, *args, **kwargs):
		#print 'in teh factory'
		nargs = bunchify(args)
		nkwargs = bunchify(kwargs)
		retval =  getattr(cls, methodname)(self, *nargs, **nkwargs)
		self.sync()
		return (retval)
	return method
...
class Bunch(dict):
	...
	def __getattr__(self, k):
		#print 'in __getattr__ of jsonBunch'
	...
	def staticToDict(self):
		global superself
		return (Bunch.toDict(superself))
	def sync(self):
		print 'saving database changes'
		wholeDB = self.staticToDict()
		keys = self.keys()
		f = open('db.json','w')
		#global printer
		#printer('wholeDB = ' + str(wholeDB))
		f.write(json.dumps(wholeDB))
		f.close()
		#print 'saved'
